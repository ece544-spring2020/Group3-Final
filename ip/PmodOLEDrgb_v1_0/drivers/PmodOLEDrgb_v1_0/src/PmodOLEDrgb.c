...
/* ------------------------------------------------------------ */
/* NOTE: Edited by Robert Holt to abstract away the conversion of
   the HSV values to RGB.  A new fuctions exists, OLEDrgb_BuildRGBFromHSV
   which now converts 8 bit HSV to 8 bit RGB.  OLEDrgb_BuildHSV is a
   convenience function which combines OLEDrgb_BuildRGBFromHSV with
   OLEDrgb_BuildRGB
*/
...
u16 OLEDrgb_BuildHSV(u8 hue, u8 sat, u8 val) {
   u8 R, G, B;
   u32 raw;
   raw = OLEDrgb_BuildRGBFromHSV(hue, sat, val);
   R = raw & 0xFF;
   G = (raw >> 8) & 0xFF;
   B = (raw >> 16) & 0xFF;
...
/*** OLEDrgb_BuildRGBFromHSV
**
**   Parameters:
**      hue - Hue of color (8 bit resolution)
**      sat - Saturation of color (8 bit resolution)
**      val - Value of color (8 bit resolution)
**
**   Return Value:
**      RGB representation of input color in 32 bit
**      representation where R, G, and B are stored with
**      8 bits of resolution.  R is in least significant
**      byte, G is in next least significant byte, and B
**      is in the next least significant byte.  The upper
**      byte does not contain meaningful information.
**
**   Errors:
**      none
**
**   Description:
**      Converts an HSV value into equivalent RGB representations
**      with the same resolution.
*/
uint32_t OLEDrgb_BuildRGBFromHSV(u8 hue, u8 sat, u8 val) {
